...
@section('title', 'Edit Blog Category - Admin Panel')

@section('content')
    <div class="mb-6">
        <a href="{{ route('admin.blog-categories.index') }}" class="text-sm text-gray-600 hover:text-primary dark:text-gray-400">← Back to Blog Categories</a>
    </div>

    <h1 class="mb-6 text-3xl font-extrabold text-secondary">Edit Blog Category: {{ $category->name }}</h1>
    ...
    <div class="max-w-2xl rounded-lg border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-700 dark:bg-gray-800">
        <form action="{{ route('admin.blog-categories.update', $category) }}" method="POST" class="space-y-6">
            ...
            <div class="flex gap-3 border-t pt-4 dark:border-gray-700">
                ...
                <a href="{{ route('admin.blog-categories.index') }}" class="rounded-md border border-gray-300 px-4 py-2 text-sm font-semibold text-gray-700 hover:bg-gray-50 dark:border-gray-600 dark:text-gray-300 dark:hover:bg-gray-700">Cancel</a>
            </div>
        </form>
    </div>
@endsection
